...
// omarchy.battery minus the automatic power-profile switch. Upstream reacts to
// UPower.onBatteryChanged with `omarchy-powerprofiles-set ac|battery`, which is
// why a hand-picked profile snapped back on every plug/unplug. The low-battery
// warning is upstream and kept. omarchy-powerprofiles-init still runs at login,
// which is a starting point at boot, not a switch that fires during use.
Item {
  id: root
