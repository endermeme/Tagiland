...
Panel {
  ...
  // Refresh rate of the focused monitor. Only shown when there is a choice.
  ...
  function sectionCount(section) {
    ...
    if (section === "refresh") return refreshRates.length
  ...
  function moveCursorH(delta) {
    var row = focusSection === "scale" ? scaleValues
            : focusSection === "refresh" ? refreshRates
            : null
    if (!row) return
  ...
  function activateCursor() {
    ...
    if (focusSection === "refresh" && selectedIndex >= 0 && selectedIndex < refreshRates.length) {
  ...
  // A ~ path handed to bash: QML has no home-directory helper without an import.
  readonly property string refreshTool: "~/.config/omarchy/bin/monitor-refresh-rate"

  function setRefreshRate(rate) {
    refreshSetProc.command = ["bash", "-c", root.refreshTool + " " + String(rate)]
    if (!refreshSetProc.running) refreshSetProc.running = true
  }

  function refreshRateIndex() {
    for (var i = 0; i < refreshRates.length; i++)
      if (refreshRates[i] === root.refreshRate) return i
    return -1
  }
  ...
  function parseRefresh(raw) {
    ...
    var current = parseInt(lines[0], 10)
    var rates = []
    for (var i = 1; i < lines.length; i++) {
      var n = parseInt(lines[i], 10)
      if (isFinite(n) && rates.indexOf(n) === -1) rates.push(n)
    }
    root.refreshRate = isFinite(current) ? current : 0
  ...
  // omarchy-monitor-state has no refresh rates, so reuse the helper the button
  // calls. First line is the current rate, the rest are what the resolution offers.
  Process {
    id: refreshProc
    command: ["bash", "-c", root.refreshTool + "; " + root.refreshTool + " --list"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.parseRefresh(text)
    }
  }
  ...
  Process {
    id: refreshSetProc
    stdout: StdioCollector { waitForEnd: true }
    onRunningChanged: if (!running) root.reloadRefresh()
  }
      ...
      ScrollView {
        ...
        Column {
          ...
          Column {
            ...
            Item {
              width: parent.width
              implicitHeight: Math.max(refreshHeader.implicitHeight, refreshHint.implicitHeight)
              ...
              // A high rate is the biggest display power draw. Say so.
              Text {
                id: refreshHint
                text: root.refreshRate >= 90 ? "USES MORE BATTERY" : ""
                color: Qt.darker(root.bar.foreground, 1.4)
                font.family: root.bar.fontFamily
                font.pixelSize: Style.font.caption
                font.bold: true
  ...
  component RefreshPill: Button {
    ...
    active: root.refreshRateIndex() === rateIndex
    hasCursor: root.cursorActive && root.focusSection === "refresh" && root.selectedIndex === rateIndex

    onClicked: root.setRefreshRate(rpill.rateValue)
    onHovered: function(isHovered) {
      if (!isHovered || root.reflowingText) return
      root.cursorActive = true
      root.focusSection = "refresh"
      root.selectedIndex = rpill.rateIndex
    }
  }
